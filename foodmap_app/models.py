...
class Offering(models.Model):
    ...
    recur = models.CharField(max_length=RECUR_MAX_LENGTH, null=True, choices=RECUR_CHOICES)
    recur_end_datetime = models.DateTimeField(null=True)
    ...
    def save(self, *args, **kwargs):
        ...
        '''
        # Enforce thread_id must be 16 chars
        ...
        if self.recur_end_datetime and self.timestamp and self.recur_end_datetime < self.timestamp:
            raise IntegrityError('\'recur_end_datetime\' attribute has value earlier than \'timestamp\'')
    ...
    def __unicode__(self):
        ...
        s = '%s %s at %s.' % (str(self.timestamp), self.title, str(self.location))
        if self.recur != None:
            s += ' Recurs %s until %s.' % (self.recur, str(self.recur_end_datetime))
        s += ' Tags: %s.' % tag_str
        return s
